...
class WcsBucketManager(object):
    ...
    def make_delete_url(self, bucket, key):
        return '{0}/delete/{1}'.format(self.mgr_host, Util.wcs_entry(bucket, key))

    def delete(self, bucket, key):
        url = self.make_delete_url(bucket, key)
        code, text = Util.do_wcs_post(url=url, headers=self.gernerate_headers(url))
        return code, text

    def make_filestat_url(self, bucket, key):
        return '{0}/stat/{1}'.format(self.mgr_host, Util.wcs_entry(bucket, key))

    def stat(self, bucket, key):
        url = self.make_filestat_url(bucket, key)
#        log.info('Start to get the stat of %s:%s', bucket, key)
        code, text = Util.do_wcs_get(url=url, headers=self.gernerate_headers(url))
#        log.info('The return code : %d and text : %s', code, text)
    ...
    def bucketlist(self, bucket, prefix=None, marker=None, limit=None, mode=None):
        ...
        code, text = Util.do_wcs_get(url=url, data=options, headers=self.gernerate_headers(url))
    ...
    def move(self, srcbucket, srckey, dstbucket, dstkey):
        ...
        logging.info('Move object %s from %s to %s' % (srckey, srcbucket, dstbucket))
        code, text = Util.do_wcs_post(url=url, headers=self.gernerate_headers(url))
        logging.info('The return code : %d and text : %s', code, text)
        return code, text
    ...
    def setdeadline(self, bucket, key, deadline):
        ...
        code, text = Util.do_wcs_post(url=url, data=body, headers=self.gernerate_headers(url, body))
        return code, text
