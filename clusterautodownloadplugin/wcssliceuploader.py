...
class WcsSliceUploader(object):
    ...
    def start_upload(self):
        """Start Upload"""
        upload_batch = Util.get_random_string()
    ...
    def _make_block(self, offset, upload_batch):
        with open(self.filepath, "rb") as openfile:
            bput = Util.readfile(openfile, offset, self.put_size)
            ...
            blkcode, blktext = self._do_post(url=url, headers=headers, data=bput)
            while self._need_repost(blkcode, blkretry, crc, blktext):
                time.sleep(2)
                blkcode, blktext = self._do_post(url=url, headers=headers, data=bput)
                blkretry = blkretry - 1
            if self._need_repost(blkcode, blkretry, crc, blktext):
                logging.warning("make block file fail, code %d, response %s"\
                , blkcode, json.dumps(blktext))
                return self._post_fail(u'MAKE_BLOCK_REQ_FAIL')
            if not blockid < self.num - 1:
                rest_block_size = self.size - (blockid * self.block_size) - self.put_size
                if not rest_block_size > 0:
                    return self._post_success(blktext['ctx'])
            offset, code, ctx = self._make_bput(openfile, blktext['ctx'], upload_batch, offset)
            if self._need_retry(code) or code != 200:
                logging.warning("make put file fail, code %d"\
                , code)
    ...
    def _make_bput(self, inputfile, ctx, upload_batch, offset):
        ...
        bput_next = Util.readfile(inputfile, offset_next, self.put_size)
        bputretry = self.put_retry
        if bput_next is None:
            logging.warning("NONE_NOFILE")
            return offset, 200, ctx
        while bput_next is not None and bputnum < self.block_size//self.put_size:
            bputcode, bputtext = self._make_bput_post(ctx, bputnum, upload_batch, bput_next)
            ...
            if self._need_repost(bputcode, 9999, crc, bputtext):
                logging.warning("upload put file fail, code %d, response %s"\
    ...
    def _make_file(self, blockstatus, upload_batch):
        ...
        while retry and self._need_retry(code):
            #log.warning('Make_file fail, now start %dth retry', mkfile_retries - retry)
            code, text = self._do_post(url=url, headers=headers, data=body)
            retry -= 1
...
#           log.info('Make_file suc! wcssliceupload complet!')
    ...
    def _do_post(self, url, headers, data):
        return Util.do_wcs_post(url, headers, data)
    ...
    def _mlkblock_url(self, offset):
        ...
        if blockid < self.num - 1:
            ...
            url = self._block_url(self.size - (blockid * self.block_size), blockid)
        return url

    def _need_retry(self, code):
        return Util.wcs_need_retry(code)
